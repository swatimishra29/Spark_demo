...
# fetch objects from exported values in datalake stack
...
export_folder_name = config.require('export_folder_name') if config.get('export_folder_name') else 'powerselect'
...
connection = glue.Connection("db-connection",
    ...
    description=f"glue db connection to powerselect db",
...
for script in scripts:
    ...
    # create glue job
    jobs[scriptname] = glue.Job(f"powerselect_{env}_{scriptname}",
        glue_version='1.0',
        command={
            'name': 'glueetl',
            'pythonVersion': '3',
        ...
        security_configuration=glue_security_config,
        timeout=60)